"""
 Title:         KFold Neural network with misorientation loss (inputs should be in radians)
...
from mms.helper.general import splice_list
...
from mms.maths.neml import get_cubic_misorientation
...
class Surrogate(__Surrogate__):
    ...
    def initialise(self, input_size:int, output_size:int, num_splits:int=5,
                   epochs:int=1000, batch_size:int=32, verbose:bool=False):
        """
        ...
        * `verbose`:     Whether to display updates or not
        ...
        # Check and initialise indexes
        if self.output_size % 3 != 0:
            raise ValueError("The 'phi_indexes' argument should contain a multiple of 3 elements!")
        self.phi_indexes_list = splice_list(list(range(self.output_size)), 3)
    ...
    def loss_function(self, target:torch.tensor, output:torch.tensor) -> torch.tensor:
        ...
        * `output`: The output values
        ...
        # Convert the tensors to unmapped sgrids (i.e., lists of lists)
        target_grid = self.output_unmapper(target.tolist())
        output_grid = self.output_unmapper(output.tolist())
        
        # Iterate through grids
        average_misorientations = []
        for target_list, output_list in zip(target_grid, output_grid):
            misorientations = []
            for phi_indexes in self.phi_indexes_list:
                target_euler = [target_list[i] for i in phi_indexes]
                output_euler = [output_list[i] for i in phi_indexes]
                misorientation = get_cubic_misorientation(target_euler, output_euler)
                misorientations.append(misorientation)
            average_misorientations.append(np.average(misorientations))

        # Convert misorientations into a tensor
        misorientation_tensor = torch.tensor(average_misorientations, requires_grad=True)
        misorientation_tensor = misorientation_tensor.mean()
        return misorientation_tensor
